...
nodedef *specify_latestnodeversion(txdef *tx, int openum) // readを行うnode, overwriteするnodeを取得
{
    ...
    {
        if (database.at(tx->txinfo.at(openum).tuple).at(v).v_cstamp < tx->t_sstamp && database.at(tx->txinfo.at(openum).tuple).at(v).status == txstatus::committed)
        {
...
void ssn_write(txdef *tx, nodedef *version, int i) // 論文に準拠
{
    ...
    tmp.tid = tx->tid;
    tmp.status = txstatus::inflight;
    pthread_mutex_lock(&dbmtx_);
    database.at(tx->txinfo.at(i).tuple).push_back(tmp); // databaseに直接追加
...
void si_commit(txdef *tx)
{
    if (tx->status == txstatus::inflight)
    {
        ...
        pthread_mutex_lock(&dbmtx_);
        // versionのcommit timestamp/statusを書き換える
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < database.at(i).size(); j++)
            {
                if (database.at(i).at(j).tid == tx->tid)
                {
                    database.at(i).at(j).v_cstamp = tx->t_cstamp;
                    database.at(i).at(j).status = txstatus::committing;
                }
            }
        }
        pthread_mutex_unlock(&dbmtx_);
...
{
    if (tx->status == txstatus::committing) // SI checkをクリア
    {
        /*if (tx->status == txstatus::inflight)
        {
    ...
    {
        tx->status = txstatus::committed; // post-commit begins
        commitcounter++;                  // transactionの処理完了フラグ
        ...
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < database.at(i).size(); j++)
            {
                if (database.at(i).at(j).tid == tx->tid)
                {
                    // database.at(i).at(j).v_prev->v_pistamp = tx->t_pistamp;
                    for (int k = 1; k < j; k++)
                    {
                        if (database.at(i).at(j - k).status == txstatus::committed)
                        {
                            database.at(i).at(j - k).v_pistamp = tx->t_pistamp;
                            break;
                        }
                    }
                    //     initialize new version
                    database.at(i).at(j).v_cstamp = database.at(i).at(j).v_etastamp = tx->t_cstamp;
                }
            }
        }
        /*for (int i = 0; i < tx->wworker.size(); i++)
...
int main()
{
    ...
    database.reserve(int(10000000000));
